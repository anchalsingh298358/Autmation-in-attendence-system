...
window = tk.Tk()
window.title("Real-Time Face Recognition based Attendance Management System")
...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        Label(ec, text='Please enter your subject name!!!', fg='2e2145', bg='white', font=('times', 16, ' bold ')).pack()
        Button(ec, text='OK', command=ec_delete, fg="black", bg="lawn green", width=9, height=1, activebackground="Red",
               font=('times', 15, ' bold '),borderwidth=10).place(x=90, y=50)
    ...
    def fill_attendance():
        ...
        cursor.execute(
            "CREATE TABLE manual_details ( Enrollment VARCHAR(40), Name VARCHAR(50), Date VARCHAR(100), TimeStamp VARCHAR(100) )")
        cursor.commit()
        print('Successful connection of database')
        Insert_data = "INSERT INTO information ( ENROLLMENT, NAME, DATE, TIME) VALUES ( %s, %s, %s, %s, %s )"
        cursor.execute(Insert_data, (ts, Date, timeStamp, Time))
        ...
        if subb == '':
            ...
            def err_screen1():
                ...
                Button(errsc2, text='OK', command=del_errsc2, fg="black", bg="lawn green", width=9, height=1,
                       activebackground="Red", font=('times', 15, ' bold '),borderwidth=10).place(x=90, y=50)
            ...
            def enter_data_DB():
                ...
                if ENROLLMENT == '':
                    ...
                    Insert_data = "INSERT INTO details ( ENROLLMENT, NAME, DATE, TIME) VALUES ( %s, %s, %s, %s, %s )"
                    VALUES = ( ENROLLMENT, STUDENT, Date, time)
                    try:
            ...
            def create_csv():
                ...
                csv_name = 'E:\Desktop\Autmation in attendence system\Attendance\Details.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer \select,"E:\Desktop\Autmation in attendence system\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def take_img():
    ...
    cursor.execute(
        "CREATE TABLE  if not exists information ( en , nm , dt , tm  )")
    Insert_data = "INSERT INTO information ( en, nm, dt, tm ) VALUES ( %s, %s, %s, %s )"
    VALUES = (l1, l2 , Date, Time )
    cursor.execute(Insert_data, VALUES)
    connection.commit()
...
def subjectchoose():
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                Insert_data = "INSERT INTO information ( en, nm, dt, tm ) VALUES ( %s, %s, %s, %s )"
                VALUES = (Subject, str(aa) ,date ,timeStamp)
                cursor.execute(Insert_data, VALUES)
                connection.commit()
                ...
                root.configure(background='snow')
                cs = 'E:/Desktop/Autmation in attendence system/' + fileName
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            ...
                            label = tkinter.Label(root, width=8, height=1, fg="black", font=('times', 15, ' bold '),
                                                  bg="lawn green", text=row, relief=tkinter.SUNKEN)
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1
                root.mainloop()
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"E:/Desktop/Autmation in attendence system/Attendance/-------Check atttendance-------"')

    attf = tk.Button(windo, text="Check Sheets", command=Attf, fg="black",bd=2, bg="lawn green", width=12, height=1,
                     activebackground="Red", font=('times', 14, ' bold '))
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bd=2,bg="deep pink", width=20,
                       height=2,
                       activebackground="Red", font=('times', 15, ' bold '),relief=SUNKEN)
    ...
    def log_in():
        ...
        if username == 'root1':
            if password == 'root1':
                ...
                cs = 'E:/Desktop/Autmation in attendence system/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg="#2e2145", fg="black", width=50,
...
AP = tk.Button(window, text="Check Register students", command=admin_panel, fg="black", bg="cyan", width=19, height=1,
               activebackground="Red", font=('times', 15, ' bold '))
...
trainImg = tk.Button(window, text="Train Images", fg="black", command=trainimg, bg="lawn green", width=20, height=3,
                     activebackground="Red", font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendace", fg="white", command=subjectchoose, bg="blue2", width=20, height=3,
               activebackground="Red", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill, fg="black", bg="lawn green",
